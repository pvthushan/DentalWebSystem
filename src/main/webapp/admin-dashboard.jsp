...
<%
    ...
    double todaysIncome = 0.00;

    try {
        Object staffAttr = request.getAttribute("activeStaff");
        if (staffAttr != null) {
            activeStaffCount = Integer.parseInt(staffAttr.toString());
            if (activeStaffCount < 0) activeStaffCount = 0;
        }

        Object apptAttr = request.getAttribute("totalAppointments");
        if (apptAttr != null) {
            totalAppointments = Integer.parseInt(apptAttr.toString());
            if (totalAppointments < 0) totalAppointments = 0;
        }

        Object incomeAttr = request.getAttribute("todaysIncome");
        if (incomeAttr != null) {
            todaysIncome = Double.parseDouble(incomeAttr.toString());
            if (todaysIncome < 0.0) todaysIncome = 0.00;
        }
...
<div class="main-content">
    <div class="header-bar">
        ...
        <div class="admin-badge">Admin: <strong><%= user != null && user.getFullName() != null ? user.getFullName() : "Administrator" %></strong></div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-btn-group">
            ...
            <a href="admin-pricing" class="btn-action btn-yellow">+ Configure Fees</a>
        </div>
    </div>
</div>
